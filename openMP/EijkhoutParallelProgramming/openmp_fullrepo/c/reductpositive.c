...
int reduce_without_zero(int r, int n)
{
	...
	{ // new value is zero: keep r
		m = r;
	}
	else if (n < r)
	{ // new value is less but not zero: use n
		m = n;
	}
	else
	{ // new value is more: use r
		m = r;
	};
#ifdef DEBUG
...
#pragma omp declare reduction(rwz:int                                           \
							  : omp_out = reduce_without_zero(omp_out, omp_in)) \
	...
	m = -1;
#pragma omp parallel for reduction(rwz \
								   : m)
	for (int idata = 0; idata < ndata; idata++)
		m = reduce_without_zero(m, data[idata]);
